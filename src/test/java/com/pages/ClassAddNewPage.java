...
public class ClassAddNewPage {
	...
	public String addingMandatoryFields(String batchName, String ClassTopic, String ClassDescription, String month,
		...
		System.out.println("batch entered");
		classTopicTextBox.click();
		System.out.println("ClassTopic clicked");
		classTopicTextBox.sendKeys(ClassTopic);
		System.out.println("ClassTopic sent");
		classDescriptionTextBox.click();
		System.out.println("classDescription box clicked");
		classDescriptionTextBox.sendKeys(ClassDescription);
		System.out.println("classDescription sent");
		...
		staffNameBoxt.click();
		System.out.println("Staffname clicked");
		...
		if (Status.equals("Active")) {
			...
			System.out.println("Inactive selected");
		}
		saveBtn.click();
...
//	    }
	...
	public void selectNoOfClasses(String Month, String date1, String date2) {
		 ...
		 * System.out.println("I entered selecting dates for loop"); String text =
		 * date.getText(); System.out.println(" date.text value "+text); if
		 * (text.equalsIgnoreCase(date1) || text.equalsIgnoreCase(date2)) {
		 * wait.until(ExpectedConditions.elementToBeClickable(date)).click();
		 * System.out.println("I clicked the date inside if block"); } }
		 */
	    ...
	    try {
	    	...
	    	System.out.println("I entered date selection method");
		    WebDriverWait wait = new WebDriverWait(candriver, Duration.ofSeconds(5));

		    wait.until(ExpectedConditions.elementToBeClickable(datePicker)).click(); //clicking the date field , calendar will display

		    while (!month.getText().contains(Month)) {
		    	System.out.println("I entered month while loop");
		        nextMonth.click();
		        System.out.println("nexMonth "+nextMonth.getText());
		    }
	    	
	    	
		
		    	System.out.println("I am inside new if block");
		        // Find the element with the matching date (date2)
		        WebElement date = candriver.findElement(By.xpath("//td[@class='ng-tns-c92-13 ng-star-inserted']/span[text()='" + date2 + "']"));
		        
		        // Click on the date element
		        date.click();
		        System.out.println("I clicked the date ");
	    	
	    	System.out.println("I entered try");
